...
import com.sqli.sapcx.checks.ParsingErrorCheck;
...
public class XmlSensor implements Sensor {
  ...
  private static final RuleKey PARSING_ERROR_RULE_KEY = RuleKey.of(SapCxItems.REPOSITORY_KEY, ParsingErrorCheck.RULE_KEY);
  ...
  public XmlSensor(FileSystem fileSystem, CheckFactory checkFactory, FileLinesContextFactory fileLinesContextFactory) {
    ...
    this.checks = checkFactory.create(SapCxItems.REPOSITORY_KEY).addAnnotatedChecks((Iterable<?>) CheckList.getCheckClasses());
    this.parsingErrorCheckEnabled = this.checks.of(PARSING_ERROR_RULE_KEY) != null;
    this.fileSystem = fileSystem;
    this.mainFilesPredicate = fileSystem.predicates().and(
      fileSystem.predicates().hasType(InputFile.Type.MAIN),
      fileSystem.predicates().hasLanguage(SapCxItems.KEY));
  }
